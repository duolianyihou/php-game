...
window.onload = function(){  
	...
	com.getData(path + "js/gambit.all.js",
		function(data){
		com.gambit=data.split(" ");
		AI.historyBill = com.gambit;
	})
	com.getData(path + "js/store.js",
		function(data){
...
com.loadImages = function(stype){
	...
	//绘制棋盘
	com.bgImg = new Image();
	com.bgImg.src  = path + "img/"+stype+"/bg.png";
	
	//提示点
	com.dotImg = new Image();
	com.dotImg.src  = path + "img/"+stype+"/dot.png";
	
	//棋子
	for (var i in com.args){
		com[i] = {};
		com[i].img = new Image();
		com[i].img.src = path + "img/"+stype+"/"+ com.args[i].img +".png";
	}
	
	//棋子外框
	com.paneImg = new Image();
	com.paneImg.src  = path + "img/"+stype+"/r_box.png";
	
	document.getElementsByTagName("body")[0].style.background= "url("+ path + "img/"+stype+"/bg.jpg)";
